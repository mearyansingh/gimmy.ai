...
export async function generateTrainingPlan(
  ...
  for (const model of models) {
    try {
      ...
      // Remove response_format as it's unreliable with many free providers
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert fitness trainer and program designer. You must respond with valid JSON only. Do not include any markdown, reasoning, or additional text. Ensure the JSON follows the requested schema.",
          },
          {
            role: "user",
            content: prompt,
      ...
      if (!content) {
        console.warn(`[AI] No content in response from model ${model}`);
        continue;
      }
      ...
      try {
        ...
        console.log(`[AI] Successfully generated plan with model: ${model}`);
        return formatPlanResponse(planData, normalizedProfile);
      } catch (parseError) {
        console.warn(`[AI] JSON Parse Failure (${model}). Content Start:`, cleanContent.substring(0, 100));
        continue;
      }
    } catch (error: any) {
      lastError = error;
      ...
      // If we hit a rate limit, we move to the next model immediately
      if (status === 429) {
        continue;
      }

      // If it's a 404 or 400 (bad model ID), definitely move to the next one
      if (status === 404 || status === 400) {
        continue;
      }

      // For other errors, continue to try different models
      continue;
    }
  }

  throw lastError || new Error("All AI models failed to generate a training plan. Please check your OpenRouter credits or try again later.");
}
